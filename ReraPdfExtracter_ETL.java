 ...
 * 
 */
class ReraPdfExtracter {

	public static void main(String[] args) throws AWTException, IOException {
		/**
		 * view source is the input or read all list from UI. Decide what is the input ?
		 * --Done TBD -Giri Store all registration numbers in a variable to iterateC:\Users\Admin\Downloads\realestate-salenium-main (12)\realestate-salenium-main\RealEstate\KA\05\02\AADYA HEIGHTS\AADYA HEIGHTS261220241257
		...
		System.out.println("Pdf File to Download if exist?: " + pdfFileNewDownload);
		System.out.println("complaint File to Download if exist?: " + complaintFileNewDownload);
		 ...
		 * (Need to keep update)
		 ***/
		//To read from source file
		List<String> regNumList1 = regNumList();
		//to read from imageOutput file
//		List<String> regNumList1 = regNumListImg();
		System.out.println("Total number of properties to process: " + regNumList1.size());
		// For testing purpose
		...
		for (int ri = 0; ri < regNumList1.size(); ri++) {
			...
			/***
			 * Level-2 Get the details from UI search if pdfStatus length is zero then it
			 * has no pdf file
			 ***/
			...
			String projectNameOrignial = driver.findElement(By.xpath("//tr/td[6]")).getText().trim();
			...
			// System.out.println(" District :" +districtName +"and Taluk :"+
			// propertyTaluk);

			// String projectNameSanitized =
			// StringUtil.ReplaceDashWithHyphen(projectNameOrignial);
			String projectName = StringUtil.ReplaceSpecialCharacters(projectNameOrignial).toUpperCase();
...
//    		CommonUtils.createFolderSubfolder(propFolderPath); 
			...
			if (projStatus.contains("APPROVED") && districtName.contains("BENGALURU URBAN") && pdfStatus.length() < 1) {
				...
				try {
					...
					try (BufferedWriter imageWriter = new BufferedWriter(imageOutputFile)) {
						...
						imageWriter.write(projectNameOrignial + "|" + districtName + "|" + propertyTaluk + "|"
								+ regNumList1.get(ri) + "|" + projectName);
						imageWriter.newLine();
			...
			/*** Common for both complaints and pdf download ***/
			// System.out.println("Project name is :" + projectName);
			String dateStr = dateFuntion();
			String projectFileName = currentDir + "\\" + propFolderPath + "\\" + projectName + dateStr;
			String folderPath = currentDir + "\\" + propFolderPath;
			...
			boolean complaintFileExists = PartialFileNameChecker.checkFileWithPartialNameAndFormat(folderPath,
					projectName, ".txt");
			if (complaintFileNewDownload.equals("YES")) {
				...
				String complaintTxt = projectFileName + ".txt";
				...
				try {
					...
					WebElement table = driver
							.findElement(By.xpath("//*[@id='menu-comp2']//table[@id='complaintList']"));
					// Find all the rows in the table (including header row)
					List<WebElement> rows = table.findElements(By.tagName("tr"));
					// Iterate through each row
					for (WebElement row : rows) {
			...
			} else if (complaintFileNewDownload.equals("NO")) {
				if (complaintFileExists) {
					System.out.println(projectName + ".txt complaint file exist and not downloading again");
					// System.out.println("-----Test Passed!-----" + regNumList1.get(ri) + " : count
					// : " + ri);
					// continue;
				} else {
					...
					try {
						...
						// WebElement table = driver.findElement(By.id("complaintList"));
						WebElement table = driver
								.findElement(By.xpath("//*[@id='menu-comp']//table[@id='complaintList']"));
			...
			// Check if pdf file already exist and the config to replace pdf or not

			boolean pdfFileExists = PartialFileNameChecker.checkFileWithPartialNameAndFormat(folderPath, projectName,
					".pdf");

			if (pdfFileNewDownload.equals("YES")) {
				...
				try {
					WebElement elementPdfClick = driver.findElement(By.xpath("//input[@onclick='generatePDF()']"));
					JavascriptExecutor ex = (JavascriptExecutor) driver;
					ex.executeScript("arguments[0].click()", elementPdfClick);
				...
				try {
					...
					WebElement shadowHost = driver.findElement(By.xpath("//print-preview-app")).getShadowRoot()
							...
							.getShadowRoot()
							.findElement(By.cssSelector("print-preview-destination-select#destinationSelect"))
							.getShadowRoot().findElement(By.cssSelector("select"));
					...
					System.out.println("Name of pdf " + projectFileName);
					char[] chArr = projectFileName.toCharArray();
					...
					System.out.println("checking new window handles and switch.....Repeat check(****)");
					Set<String> handlers = driver.getWindowHandles();
					Iterator itr = handlers.iterator();
					String parent = (String) itr.next();
					String child = (String) itr.next();
			...
			} else if (pdfFileNewDownload.equals("NO")) {
				if (pdfFileExists) {
					System.out.println(projectName + ".pdf file exist and not downloading again");
					System.out.println("-----Test Passed!-----" + regNumList1.get(ri) + " : count : " + ri);
					continue;
				} else {
					String viewProjDetail = "//tr[td[contains(text()," + "'" + regNumList1.get(ri) + "'"
					...
					try {
						...
						waiting(3000);
						System.out.println("inside No such element exception****on print option");
						try {
							WebElement elementPdfClick = driver.findElement(By.xpath("//input[@onclick='generatePDF()']"));
							JavascriptExecutor ex = (JavascriptExecutor) driver;
							ex.executeScript("arguments[0].click()", elementPdfClick);
					...
					try {
						...
						System.out.println("Selecting save as option.....");
						WebElement shadowHostSave = driver.findElement(By.cssSelector("print-preview-app"))
								.getShadowRoot().findElement(By.cssSelector("print-preview-sidebar#sidebar"))
								.getShadowRoot().findElement(By.cssSelector("print-preview-button-strip"))
								.getShadowRoot().findElement(By.cssSelector("cr-button.action-button"));
						...
						System.out.println("got ******NoSuchElementException**** and is going to handle ");
						try {
							WebElement elementPdfClick = driver.findElement(By.xpath("//input[@onclick='generatePDF()']"));
							JavascriptExecutor ex = (JavascriptExecutor) driver;
							ex.executeScript("arguments[0].click()", elementPdfClick);
	...
	//Read from imageOutput file
	public static ArrayList<String> regNumListImg() throws IOException {

		String filePath = "ImageOutputData.txt";
		ArrayList<String> regNolist = new ArrayList<String>();
